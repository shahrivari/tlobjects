...
public final class StreamUtils {
    ...
    public static void writeByteArrayWithSize(byte[] data, OutputStream stream) throws IOException {
        writeInt(data.length, stream);
        stream.write(data);
    }

    public static void writeByteArray(byte[] data, int offset, int len,
    ...
    public static <T extends TLObject> T readTLObject(InputStream stream, TLContext context,
                                                      Class<T> clazz,
                                                      int constructorId) throws IOException {
    ...
    public static byte[] readByteArray(InputStream stream) throws IOException {
        return readBytes(readInt(stream), stream);
    }
    ...
    public static TLIntVector readTLIntVector(InputStream stream,
                                              TLContext context) throws IOException {
        return context.deserializeIntVector(stream);
    }

    public static TLLongVector readTLLongVector(InputStream stream,
                                                TLContext context) throws IOException {
        return context.deserializeLongVector(stream);
    }

    public static TLStringVector readTLStringVector(InputStream stream,
                                                    TLContext context) throws IOException {
    ...
    public static void writeInts(int[] ints, OutputStream stream) throws IOException {
        writeInt(ints.length, stream);
        for (Integer integer : ints)
            writeInt(integer, stream);
    }

    public static void writeInts(Iterable<Integer> ints, OutputStream stream) throws IOException {
        int size;
        if (ints instanceof Collection) {
            size = ((Collection<?>) ints).size();
        } else {
            size = (int) StreamSupport.stream(ints.spliterator(), false).count();
        }

...
object TlPoet {
    // Avoid to put it as nullable
    private var tlDefinition = TLDefinition(emptyMap(), emptyList(), emptyList())
    // Map a type to it's ClassName to reference inside code
    val typeClassNameMap = HashMap<TLType, ClassName>()
    // Map a constructor to it's className to use when building context
    val constructorClassNameMap = HashMap<TLConstructor, ClassName>()
    // Map a constructor to true if it's abstracted for an "Empty" constructor only
    val emptyConstructorAbstractedMap = HashMap<TLConstructor, Boolean>()
    // List of constructors to register in context
    val contextConstructors = ArrayList<TLConstructor>()
    // List of types for test context (contextConstructors + method)
    val testContext = ArrayList<ClassName>()

    var totalClass = 0

    private val annotationSpec = AnnotationSpec.builder(SuppressWarnings::class.java)
            .addMember("value", "{\"unused\", \"unchecked\", \"RedundantCast\"}").build()
    ...
    fun writeClasses(tlDefinition: TLDefinition) {
        TlPoet.tlDefinition = tlDefinition
    ...
    private fun writeConstructors() {
        ...
        constructors.map { it.tlType }.forEach {
            typeOccurrences.put(it, typeOccurrences.getOrDefault(it, 0) + 1)
        }
        // No need for abstraction
        val nonAbstractedConstructors = constructors.filter { typeOccurrences[it.tlType]!! == 1 }
        // Need abstraction
        val abstractedConstructors = constructors.filter { typeOccurrences[it.tlType]!! > 1 }
        // Fake constructor created by us
        val abstractConstructors = ArrayList<TLAbstractConstructor>()
        ...
        for (abstractType in abstractedConstructors.map { it.tlType }.distinct()) {
            ...
            val abstractEmptyConstructor =
                    typeConstructors.size == 2 &&
                    typeConstructors.map {
                        it.name.endsWith("empty", true)
                    }.contains(true)
            if (abstractEmptyConstructor)
                typeConstructors.forEach { emptyConstructorAbstractedMap.put(it, true) }

            val commonParameters = typeConstructors.map { it.parameters }
                    .reduce { l1, l2 -> ArrayList(l1.intersect(l2)) }
            abstractConstructors.add(TLAbstractConstructor(abstractType.name,
                                                           commonParameters,
                                                           abstractType,
                                                           abstractEmptyConstructor))
            commonParameters.forEach { it.inherited = true }

            // Update each types parameters: reference are not the same
            typeConstructors.flatMap { it.parameters }
                    .filter { commonParameters.contains(it) }
                    .forEach { it.inherited = true }
        }
        ...
        abstractConstructors.forEach { constructor ->
            typeClassNameMap.put(constructor.tlType,
                                 ClassName.get(packageName(constructor.name),
                                               className(constructor.name,
                                                         abstract = true)))
        }
        abstractedConstructors.forEach { constructor ->
            constructorClassNameMap.put(constructor, ClassName.get(packageName(constructor.name),
                                                                   className(constructor.name)))
        }

        // Build classes
        nonAbstractedConstructors.forEach { constructor ->
            writeClassToFile(packageName(constructor.name),
                             generateConstructorClass(constructor))
        }
        abstractConstructors.forEach { constructor ->
            writeClassToFile(packageName(constructor.name),
                             generateAbstractConstructorClass(constructor))
        }
        abstractedConstructors.forEach { constructor ->
            writeClassToFile(packageName(constructor.name),
                             generateConstructorClass(constructor,
                                                      typeClassNameMap[constructor.tlType]!!))
        }
    }
    ...
    private fun writeMethods() {
        ...
        println("${methods.size} method related classes")

        val parameterizedTypeName = ParameterizedTypeName.get(TYPE_TL_METHOD, TypeVariableName.get("T"))
        apiWrappedClazz.addMethod(MethodSpec.methodBuilder("executeRpcQuery")
                                          ...
                                          .addParameter(parameterizedTypeName, "method")
                                          .build())

        methods.forEach { method -> writeClassToFile(PACKAGE_TL_API_REQUEST, generateMethodClass(method)) }

        writeClassToFile(PACKAGE_TL_API, apiClazz.build(), JAVA_SOURCE_OUTPUT)
        writeClassToFile(PACKAGE_TL_API, apiWrappedClazz.build(), JAVA_SOURCE_OUTPUT)
        totalClass += 2
    }
    ...
    private fun writeContext() {
        ...
        contextClazz.addMethod(MethodSpec.constructorBuilder()
                                       .addModifiers(Modifier.PUBLIC)
                                       .addStatement("super(\$L)", contextConstructors.size)
                                       .build())
        ...
        val methodBuilder = MethodSpec.methodBuilder("init").addModifiers(Modifier.PUBLIC)
    ...
    private fun writeTestContext() {
        ...
        contextClazz.addMethod(MethodSpec.constructorBuilder()
                                       .addModifiers(Modifier.PUBLIC)
                                       .addStatement("super(\$L)", testContext.size)
                                       .build())

        contextClazz.addField(TYPE_TL_API_TEST_CONTEXT,
                              "instance",
                              Modifier.PRIVATE,
                              Modifier.STATIC)
        ...
        for (clazzType in testContext.sortedBy { it.simpleName() }) {
            methodBuilder.addStatement("registerClass(\$T.CONSTRUCTOR_ID, \$T.class" + ")",
                                       clazzType,
                                       clazzType)
        }
        contextClazz.addMethod(methodBuilder.build())

        writeClassToFile(PACKAGE_TL_API, contextClazz.build(), JAVA_TEST_OUTPUT)
        totalClass++
    }
    ...
    private fun generateAbstractConstructorClass(constructor: TLAbstractConstructor): TypeSpec {
        ...
        constructors.forEach {
            clazz.addJavadoc("<li>{@link ${constructorClassNameMap[it].toString()
                    .substringAfterLast('.')}}: ${it.name}#${hex(it.id)}</li>\n")
        }
        clazz.addJavadoc("</ul>\n\n")

        if (constructor.abstractEmptyConstructor) {
            val nonEmptyConstructor = constructors.find { c ->
                !c.name.endsWith("empty", true)
            }!!

            val emptyConstructor = constructors.find { c ->
                c.name.endsWith("empty", true)
            }
            ...
            val name = nonEmptyConstructor.name.split(".").last().uCamelCase()
            clazz.addMethod(MethodSpec.methodBuilder("getAs$name")
                                    .addModifiers(Modifier.PUBLIC)
            ...
            if (constructor.name.startsWith("input", true)) {
                clazz.addMethod(MethodSpec.methodBuilder("newEmpty")
                                        ...
                                        .addStatement("return new \$T()",
                                                      constructorClassNameMap[emptyConstructor])
                                        .build())
                ...
                clazz.addMethod(MethodSpec.methodBuilder("newNotEmpty")
                                        ...
                                        .returns(constructorClassNameMap[nonEmptyConstructor])
                                        .addStatement("return new \$T()",
                                                      constructorClassNameMap[nonEmptyConstructor])
                                        .build())
            }
        }

        return clazz.addJavadoc(JAVADOC).build()
    }

    private fun generateConstructorClass(constructor: TLConstructor,
                                         superclass: ClassName = TYPE_TL_OBJECT): TypeSpec {
        ...
        if (emptyConstructorAbstractedMap.getOrDefault(constructor, false)) {
            clazz.addMethod(MethodSpec.methodBuilder("isEmpty")
                                    ...
                                    .addStatement("return \$L",
                                                  constructor.name.endsWith("empty", true))
                                    .build())
            ...
            if (!constructor.name.endsWith("empty", true)) {
                val name = constructor.name.split(".").last().uCamelCase()
                clazz.addMethod(MethodSpec.methodBuilder("getAs" + name)
                                        .addModifiers(Modifier.PUBLIC, Modifier.FINAL)
                                        .addAnnotation(Override::class.java)
    ...
    private fun generateMethodClass(method: TLMethod): TypeSpec {
        ...
        val deserializeResponseMethod = MethodSpec.methodBuilder("deserializeResponse")
                ...
                .addAnnotation(AnnotationSpec.builder(SuppressWarnings::class.java)
                                       .addMember("value",
                                                  """{"unchecked", "SimplifiableConditionalExpression"}""")
                                       .build())
        ...
        if (method.tlType is TLTypeAny) {
            ...
            // Delegate
            deserializeResponseMethod.addStatement("return query.deserializeResponse(stream, context)",
            ...
            deserializeResponseMethod.addStatement("final \$T response = readTLObject(stream, context)", TYPE_TL_OBJECT)
                    ...
                    .beginControlFlow("if (!(response instanceof \$T))",
                                      if (responseType is ParameterizedTypeName) responseType.rawType
                                      else responseType)
                    .addStatement("throw new \$T(\"Incorrect response type, expected \" " +
                                  "+ getClass().getCanonicalName() + \", found \" " +
        ...
        apiWrappedMethod = MethodSpec.methodBuilder(method.name.lCamelCase())
                ...
                .addStatement("return (\$T) executeRpcQuery(new \$T(\$L))",
                              ...
                              if (method.parameters.isNotEmpty())
                                  method.parameters.filterNot { it.tlType is TLTypeFlag }.map {
                                      it.name.lCamelCase()
                                              .javaEscape()
                                  }.joinToString(", ")
                              else "")
    ...
    private fun generateClassCommon(clazz: TypeSpec.Builder,
                                    clazzTypeName: ClassName?,
                                    name: String,
                                    id: Int?,
                                    parameters: List<TLParameter>) {
        if (id != null) {
            // CONSTRUCTOR_ID field
            clazz.addField(FieldSpec.builder(TypeName.INT, "CONSTRUCTOR_ID",
            ...
            for (parameter in condParameters) {
                ...
                if (realType is TLTypeRaw && arrayOf("true", "false").contains(realType.name)) {
                    ...
                    if (condParameters.any {
                                (it != parameter
                                 && !((it.tlType as TLTypeConditional).realType is TLTypeRaw &&
                                      (it.tlType.realType as TLTypeRaw).name == "Bool")
                                 && (it.tlType as TLTypeConditional).value == tlType.value)
                            })
                        condBoolean.add(parameter)
                } else {
                    if (realType is TLTypeRaw && realType.name == "Bool" &&
                        condParameters.any {
                            it != parameter && (it.tlType as TLTypeConditional).value == tlType.value
                        }) {
        ...
        for (parameter in parameters) {
            ...
            // Don't addFileChunk if flags, since it'll be computed
    ...
    private fun serializeParameter(fieldName: String, fieldTlType: TLType): String = when (fieldTlType) {
        ...
        is TLTypeConditional -> {
            ...
            if (fieldTlType.realType !is TLTypeRaw || fieldTlType.realType.name != "Bool") {
                statement.append("""if ($fieldName == null) throwNullFieldException("$fieldName", flags);""")
                        .append('\n')
                        .append("    ")
            }
